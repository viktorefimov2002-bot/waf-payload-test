...
def build_json(value: str, structure: str, depth: int, width: int, fields: int, ensure_ascii: bool = False) -> Document:
...
def build_xml(value: str, structure: str, depth: int, width: int) -> Document:
    ...
    if structure == "single":
        ...
        validity, metrics = "valid", {"depth": 2, "width": 1, "fields": 1}
    elif structure == "deep":
        ...
        validity, metrics = "valid", {"depth": depth + 1, "width": 1, "fields": depth}
    elif structure == "wide":
        children = "".join(f'<item id="{index}">{safe}</item>' for index in range(width))
        body = f'<?xml version="1.0"?><root>{children}</root>'
        validity, metrics = "valid", {"depth": 2, "width": width, "fields": width}
        ...
        validity, metrics = "invalid", {"depth": 2, "width": 1, "fields": 1}
    else:
...
def build_multipart(value: str, structure: str, fields: int, boundary: str) -> Document:
    ...
    for index in range(count):
        ...
        parts.extend([f"--{boundary}", f'Content-Disposition: form-data; name="{name}"', "", value if structure == "single" else f"{value}-{index}"])
...
def build_document(fmt: str, value: str, value_encoding: str, structure: str, depth: int, width: int, fields: int, boundary: str) -> Document:
    if fmt == "json":
        return build_json(value, structure, depth, width, fields, ensure_ascii=value_encoding == "json-unicode-escape")
    if fmt == "form":
                    ...
                    for filler_kind in args.filler_kinds:
                        for requested_size in args.sizes:
                            ...
                            for value_encoding in value_encodings_for(fmt, structure, args):
                                ...
                                document = build_document(fmt, encoded_value, value_encoding, structure, args.depth, args.width, args.fields, boundary)
                                ...
                                for compression in compressions:
                                    ...
                                    for corruption in corruption_modes:
                                        ...
                                        case_number += 1
                                        case_id = f"case-{case_number:06d}-{fmt}-{structure}-{value_encoding}-{charset}-{compression}-{corruption}"
                                        headers = {
                                        ...
                                        metadata = {
                                            ...
                                            "encoded_value_utf8_size": len(encoded_value.encode("utf-8")),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--value-encoding-profile", choices=["plain", "recommended"], default="plain", help="Format-aware value encoding profile")
    parser.add_argument("--value-encodings", nargs="+", choices=ALL_VALUE_ENCODINGS, help="Explicit encodings; unsupported format combinations are skipped")
