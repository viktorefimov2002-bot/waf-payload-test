...
def test_every_generated_case_has_exact_sha256():
    for case in decompression_profile.iter_cases(args()):
        ...
        assert case["metadata"]["stress_profile"] == "phase2"
...
def test_concatenated_gzip_members_restore_original_size():
    cases = list(decompression_profile.iter_cases(args(algorithms=["gzip"], variants=["gzip-members"])))
    assert cases
    case = cases[0]
    wire = base64.b64decode(case["body_base64"])
...
def test_sync_flush_metadata_is_recorded():
    cases = list(decompression_profile.iter_cases(args(algorithms=["deflate"], variants=["sync-flush"], flush_chunk_sizes=[32])))
    assert cases[0]["metadata"]["flush_chunk_size"] == 32


def test_nested_header_matches_chain_order():
    cases = list(decompression_profile.iter_cases(args(algorithms=["gzip"], variants=["nested-same"], nested_depths=[3])))
    case = cases[0]
