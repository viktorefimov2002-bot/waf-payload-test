...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", default="payloads_decompression_stress.jsonl")
    parser.add_argument("--path", default="/decompression-test")
    parser.add_argument("--formats", nargs="+", choices=["json", "text"], default=["json"])
    parser.add_argument("--algorithms", nargs="+", choices=_decompression_profile.ALGORITHMS, default=["gzip", "deflate", "raw-deflate"])
    parser.add_argument("--variants", nargs="+", choices=_decompression_profile.VARIANTS, default=["standard", "gzip-members", "sync-flush", "nested-same"])
    parser.add_argument("--decompressed-sizes", nargs="+", type=int, default=_decompression_profile.DEFAULT_DECOMPRESSED_SIZES, metavar="BYTES")
...
def iter_cases(args: argparse.Namespace) -> Iterable[dict[str, Any]]:
    args.stress_profile = LEGACY_PROFILE
    for case in _decompression_profile.iter_cases(args):
